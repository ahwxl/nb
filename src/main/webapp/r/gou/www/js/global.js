...
Tools.isTel = function(A) {
    //var B = /^[\d|\-|\s|\_]+$/;
	var B = /^[0-9- ]{7,20}$/; // 7到20位 数字验证
    return B.test(A)
};
Tools.isMobile = function(A) {
    var B = /^13[0-9]{9}|15[012356789][0-9]{8}|18[0256789][0-9]{8}|147[0-9]{8}$/;
    return B.test(A)
};
...
function collectGoodsAction(A) {
    $.ajax({
        ...
        cache: false,
        data: "id=" + A + "&backurl=goods.php?id=" + A,
        success: function(C) {
            C = $.evalJSON(C);
            alert(C.message);
            if (C.error == "1") {
                var B = encodeURIComponent("http://www.quwan.com/goods.php?id=" + A);
                location.href = "http://www.quwan.com/user.php?action=collect&id=" + A + "&backurl=" + B
            }
        }
    })
}
...
function payEndAction(A, B, C) {
    if (B == "hzly" || B == "bocom" || B == "kuaiqian" || B == "yunwang" || B == "cmb" || B == "icbc" || B == "abc" || B == "ccb" || B == "bcom" || B == "sdb" || B == "gdb" || B == "citic" || B == "hxb" || B == "cib" || B == "ceb" || B == "tenpay" || B == "soopay" || B == "boc_sh" || B == "boc_gz"||  B == "spdb" || B == "tenpayb2c_quwan") {
        $("#" + C)[0].submit()
    } else {
        window.open(A)
    }
...
function getApiUrl(A)
{
    if(A == '1' || A == '2' || A == '3' || A == '4' || A == '5' || A == '6' || A == '7')
    {
		...
		if(A == '4' || A == '6' || A == '3' || A =='7')
		{
			if(document.getElementById('back_act').value == '')
				var bakurl = document.location.href;
			else
				var bakurl = document.getElementById('back_act').value;
			window.location.href = WEB + 'united_login.php?ulapitype='+A+'&B='+bakurl+'&m=' + Math.random();
		}
		else
		{
        	window.open(WEB + 'united_login.php?ulapitype='+A+'&m=' + Math.random());
        }
    }else
    {
        return false;
